...
class CoordinatorSpec extends ScalaTestWithActorTestKit with AnyWordSpecLike {
  ...
  "3 working coordinators and 1 failed coordinator" must {
  ...
  def spawnAll(nCoordinators: Int, nCommittingParticipants: Int, nAbortingParticipants: Int = 0, nFailedCoordinators: Int = 0): (Array[Messages.Coordinator], Array[Messages.Participant]) = {
    val cs = new Array[Messages.Coordinator](nCoordinators + nFailedCoordinators)
    ...
    for (x <- 0 until nCoordinators) {
      cs(x) = spawn(Coordinator(genSignedKey(kpg, masterKey), masterKey.getPublic(),true), testNr + "Coordinator-" + x)
    }
    for (x <- nCoordinators until nCoordinators + nFailedCoordinators) {
      cs(x) = spawn(Coordinator(genSignedKey(kpg, masterKey), masterKey.getPublic(), false), testNr + "Coordinator-" + x)
    }
