...
object ExecutionPlan {
  ...
  def createPlan(
      ...
      taskIDCounters: ArrayBuffer[Int]
  ): Array[ArrayBuffer[(Int, Int)]] = {
    // Plan holds for each operator the tuple (TM, TaskID)
    val plan = Array.fill(ops.length + 1)(
      ArrayBuffer.empty[(Int, Int)]
    ) // 1 extra row for data sources

    val dataSources = Array(
      dataSource
    ) // indices of taskManagers who will provide data
  ...
  def equalPlans(
      plan1: Array[ArrayBuffer[(Int, Int)]],
      plan2: Array[ArrayBuffer[(Int, Int)]]
  ): Boolean = {
