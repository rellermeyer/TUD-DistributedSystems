...
object FeatureTestNActorGnutella {

  private val r = scala.util.Random

  import gia.core.GnutellaActor._
  ...
  def main(args: Array[String]): Unit = {
    ...
    val superActor = superSystem.actorOf(SuperActorGnutella.props(superIp))
    ...
    for (i <- 0 until nodes) {
      ...
      // Create the actor
      val actor = system.actorOf(GnutellaActor.props(ip, port, capacity, superActor), "gnutellaActor")
    ...
    for (i <- 0 until iterations) {
      ...
      superActor ! SuperActorGnutella.Search(file, superActor)
      Thread.sleep(2*1000)
      superActor ! SuperActorGnutella.Report()
    }

    println("done")
    superActor ! SuperActorGnutella.ReportStatistics(iterations, replication)
  }
