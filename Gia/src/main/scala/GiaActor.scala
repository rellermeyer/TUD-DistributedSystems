...
class GiaActor(ip: String, port: Int, capacity: Int) extends Actor with ActorLogging with Timers {
  ...
  timers.startPeriodicTimer(TriggerGrantTokenKey, TriggerGrantToken, 60.second)
  timers.startPeriodicTimer(TriggerReplicateToNeighborKey, TriggerReplicateToNeighbor, 60.second)
...
object GiaActor {
  ...
  def props(ip: String, port: Int, capacity: Int): Props = Props(new GiaActor(ip, port, capacity))
