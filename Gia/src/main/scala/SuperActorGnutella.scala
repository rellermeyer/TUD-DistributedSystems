...
class SuperActorGnutella(ip: String) extends Actor with ActorLogging with Timers {

  import SuperActorGnutella._
  ...
  override def receive: Receive = {
    ...
    case Report() => {
      println(localCount + " " + relayCount)

      completeMessageCount += localCount +  relayCount
      completeFilesFound += localCount
  ...
  private def search(actorRef: ActorRef, file: String, origin: ActorRef): Unit = {
    actorRef ! GnutellaActor.Search(file, 32, new mutable.HashSet[ActorRef](), origin)
  }

}

object SuperActorGnutella {

  def props(ip: String): Props = Props(new SuperActorGnutella(ip))
