...
object TestInsertion {
  def insertionTest(numberOfNodes: Int, numberOfFiles: Int, numberOfKnownGroupEntries: Int = 2, verbose: Boolean): Unit = {
    assert(numberOfKnownGroupEntries > 1)
    val system = ActorSystem("KelipsInsertionTest")
    val writer = system.actorOf(Props(new Writer("testResults", s"insertLatencyn=${numberOfNodes}f=$numberOfFiles.txt", "insertion")))
  ...
  def main(args: Array[String]): Unit = {
    insertionTest(100, 100, verbose = false)
  }
}
