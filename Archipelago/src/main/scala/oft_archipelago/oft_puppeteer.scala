...
class Puppeteer extends Actor {
  ...
  def receive: PartialFunction[Any, Unit] = {
    ...
    case ("new_round",procStats:ArrayBuffer[Int]) =>
      ...
      for (i <- 0 until Main.N) {
        if(procStats(i) == 1)
          Main.nodes(i) ! ("enabled",procStats)
        else{
          Main.nodes(i) ! ("disabled",procStats)
        }
      }
    case "already_decided" =>
      alreadyDecided += 1
      if(alreadyDecided == Main.N){
        println("looks like all have converged already....stopping simulation early",println(alreadyDecided))
      }
