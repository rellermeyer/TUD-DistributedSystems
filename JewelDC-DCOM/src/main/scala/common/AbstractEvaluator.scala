...
abstract class AbstractEvaluator(registry: Registry,
                                 var eventBuffers: Map[String, mutable.MutableList[Event]] = Map()) extends Evaluator {

  override def evaluateBinaryExpression(activity: String,
                               left: Expression,
                               right: Expression,
                               scalarClock: Int): AResult = {
    val eventBuffer = eventBuffers.get(activity) match {
      case Some(eb) => eb
      case None =>
        // If there is no Event Buffer for the given activity, create a new one.
        val eb = new mutable.MutableList[Event]()
        eventBuffers += activity -> eb
        eb
    }
  ...
  override def flushEventBuffer(activity: String): Array[Event] = {
    ...
    eventBuffers -= activity
    result match {
      case None => throw new RemoteException("No activity with name \"" + activity + "\" that name logged on " + getName)
  ...
  /**
    ...
    */
  private def subtractor(): Evaluator = cachedAdder match {
    case None =>
  ...
  /**
    ...
    */
  private def multiplier(): Evaluator = cachedAdder match {
    case None =>
